...
@staticmethod
def add_random_resource(model, quantity=None):
    """
    ...
    if quantity == None:
        model.create_agent(Resource, (x, y), quantity=model.resource_config.QUANTITY)
    else:
        model.create_agent(Resource, (x, y), quantity=quantity)

@staticmethod
def add_resource_in_distance(model, distance: float, quantity=None):
    """
...
@staticmethod
def add_n_resources_in_angle_range(model, distance: float, n_resources: float, max_angle: float, quantity=None):
    assert distance > (HC.RADIUS + RC.RADIUS), "Resources should not overlap with the hive."
