...
class Hive(Agent):
    ...
    def feed_bees(self):
        ...
        bees_in_hive = [other_agent for other_agent in self.model.schedule.agents if other_agent != self and ((other_agent.x - self.location[0])**2 + (other_agent.y - self.location[1])**2)**0.5 <= (self.radius*1.5) and isinstance(other_agent, Bee)]
